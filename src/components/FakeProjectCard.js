...
const FakeProjectCard = ({ color, title, desc, date, image, link }) => {
    
    let bullets = [];
    for (let item of desc.split("|")) {
        bullets.push(
            <p className={`${WalsheimThin.className} text-textcolor text-lg mb-[5px]`}>{item}</p>
        )
    }
